...
Deliver the requested annotated MP4 only, preserving the requested clip range and source dimensions/FPS/audio unless the user asks otherwise. Do not require STT/SRT when speech timing was not edited. Render only the requested excerpt (for example, the first 10 seconds) instead of re-encoding the full source.
...
Do **not** hand the user raw STT/OCR metadata, overlay frames, intermediate 4K masters, face tracks, analysis WAVs, waveforms, sample frames, or join previews unless they explicitly ask to keep working files.
...
project/
  ...
  work/                         # TEMP / reproducible edit (deletable)
    ...
    graphics/                   # job JSON, OCR tracks, previews, overlay frames
```

During the job, put all samples, waveforms, spectrograms, RMS dumps, face contact sheets, join WAVs, OCR output, contact sheets, overlay frames, and intermediate encodes under `work/`.

## Cleanup contract

**Before finishing the task**, always:

1. Confirm `deliverables/` contains exactly what the user should keep: MP4 + SRT for editing, requested MP4 for graphics-only, or only requested SRT/VTT/TXT for caption/transcript-only.
2. Run the cleanup for the active mode:
   ```bash
   # video-editing job
   bash scripts/cleanup_work.sh --project-root . --also-masters

   # graphics-only job, after requested MP4 is in deliverables/
   bash scripts/cleanup_work.sh --project-root . --graphics-work
   ...
   ```
   Editing cleanup removes analysis media, face intermediates, joins, previews, and duplicate clean masters while keeping lean `work/edit/*.json`. Mode flags remove `work/graphics/` or `work/captions/`. All preserve `deliverables/`.
...
Default graphics-only names describe the requested clip/effect without replacing the source:

```text
deliverables/<source-stem>-<effect>-<start>-<end>.mp4
```
