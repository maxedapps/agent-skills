...
function usage() {
  console.log(`Render a Markdown code-review report to self-contained HTML with a copy button.

Usage:
  node scripts/render-review-html.mjs review.md --out review.html [--open]
...
const outputPath = resolve(
  outIndex >= 0 ? args[outIndex + 1] : inputPath.replace(/\.md$/i, '') + '.html',
)
...
const title = escapeHtml(markdown.match(/^#\s+(.+)$/m)?.[1]?.trim() || 'Code review report')
...
`
  ...
  console.error(
    `Warning: Bun markdown rendering unavailable (${reason}). Falling back to escaped plain text.`,
  )
...
main { max-width: 980px; margin: 48px auto; padding: 48px 56px; background: #fff; border: 1px solid #e5e7eb; border-radius: 18px; box-shadow: 0 18px 50px rgba(15, 23, 42, 0.08); }
...
@media (prefers-color-scheme: dark) {
  ...
  h1, h2, h3, h4, strong { color: #f9fafb; }
...
<body>
<div class="copy-button-wrap"><button class="copy-button" type="button" data-copy-response>Copy report</button></div>
<main>
