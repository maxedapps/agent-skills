...
- Prefer a fresh-context, read-only independent reviewer for the final draft when safely available and worthwhile. Ask for independent judgment, not approval. If none is available or worthwhile, record the independence limitation and perform a separate checklist-driven direct review; do not block plan completion solely on delegation.
...
When an independent reviewer is safely available and worthwhile, provide the exact request, complete draft, planning-memory path or non-persisted support, source artifacts, decisions and rationale, constraints and non-goals, known risks and assumptions, unresolved questions, and relevant code/test pointers. Require fresh-context, read-only inspection and independent judgment rather than confirmation. Ask for missing requirements/files, unsafe assumptions, weak rationale, incomplete snippets/checks/review guidance, false-green checkpoints, and unnecessary repetition.

Evaluate every finding critically. Record accepted findings, rejected findings with rationale, revisions, unresolved concerns, and review identity or direct-review limitation in `.progress` when writes permit. Revise without copying the transcript into the plan, rerun the quality checklist and affected evidence checks, and request same-assignment follow-up from the same reviewer after material revisions when practical. If independent critique is unavailable or not worthwhile, perform and record a separate checklist-driven direct review with the independence limitation. Completion requires material concerns to be resolved, documented as a reasoned disagreement, or exposed as a human decision; delegation alone is never a blocker.
